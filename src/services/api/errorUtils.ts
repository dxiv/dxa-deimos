 ...
 */
const CONNECTIVITY_DOCTOR_HINT = ' · /doctor or deimos doctor for diagnostics.'
...
function redactSensitivePatterns(message: string): string {
  return message
    .replace(/Bearer\s+[\w._-]+/gi, 'Bearer [REDACTED]')
    .replace(/\bsk-ant-[a-zA-Z0-9_-]{8,}/gi, '[REDACTED]')
}

/** Caps length and strips common secret patterns from strings shown to users. */
export function capUserFacingDetail(message: string, maxLen = 800): string {
  const cleaned = redactSensitivePatterns(message)
...
export function formatAPIError(error: APIError): string {
  ...
  if (connectionDetails) {
    ...
    if (code === 'ETIMEDOUT') {
      return `Request timed out. Check your internet connection and proxy settings${CONNECTIVITY_DOCTOR_HINT}`
    }
    ...
    if (isSSLError) {
      switch (code) {
        ...
        default:
          return `Unable to connect to API: SSL error (${code})${CONNECTIVITY_DOCTOR_HINT}`
      }
    }
  }
    ...
    if (connectionDetails?.code) {
      return `Unable to connect to API (${connectionDetails.code})${CONNECTIVITY_DOCTOR_HINT}`
    }
    return `Unable to connect to API. Check your internet connection${CONNECTIVITY_DOCTOR_HINT}`
  }
  ...
  if (!error.message) {
    ...
    return (
      (nested ? capUserFacingDetail(nested) : null) ??
  ...
  if (sanitizedMessage !== error.message && sanitizedMessage.length > 0) {
