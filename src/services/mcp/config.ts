...
import { fetchDeimosCloudMcpConfigsIfEligible } from './anthropicCloudMcp.js'
 ...
 * in getDeimosMcpConfigs(): --mcp-config (main.tsx) and the mcp_set_servers
...
export async function addMcpConfig(
  ...
  if (isDeimosInChromeMCPServer(name)) {
...
export function getMcpConfigByName(name: string): ScopedMcpServerConfig | null {
  ...
  // by name. User/project/local servers are blocked — same as getDeimosMcpConfigs().
 ...
 */
export async function getDeimosMcpConfigs(
