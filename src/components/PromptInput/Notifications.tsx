...
import { useDeimosUsageLimits } from '../../services/subscriberUsageLimitsHook.js';
...
export function Notifications(t0) {
  ...
  const deimosUsageLimits = useDeimosUsageLimits();
  let t5;
  let t6;
  ...
  const isInOverageMode = deimosUsageLimits.isUsingOverage;
  let t7;
  ...
  return <>
      ...
      {(apiKeyStatus === 'invalid' || apiKeyStatus === 'missing') && <Box>
          <Text color="error" wrap="truncate">
            {isEnvTruthy(process.env.DEIMOS_REMOTE) ? 'Authentication error · Try again' : 'Not logged in · Run /login'}
          </Text>
        </Box>}
      {debug && <Box>
          <Text color="warning" wrap="truncate">
      ...
      <TokenWarning briefOnly={isBriefOnly} tokenUsage={tokenUsage} model={mainLoopModel} />
