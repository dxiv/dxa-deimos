...
const missingCodexAuthPath = join(tmpdir(), 'dxa-agent-missing-codex-auth.json')
...
test('codex launch prefers auth account id over stale persisted value', async () => {
  const codexHome = mkdtempSync(join(tmpdir(), 'dxa-agent-codex-'))
  try {
    writeFileSync(
...
test('saveProfileFile writes a profile that loadProfileFile can read back', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'dxa-agent-profile-file-'))
