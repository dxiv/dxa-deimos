        ...
        try {
          ...
          async function handleInboundMessage(msg: SDKMessage): Promise<void> {
            try {
              ...
              if (!fields) return;
              const { uuid, content: inboundContent } = fields;

              // Dynamic import keeps the bridge code out of non-BRIDGE_MODE builds.
              const {
                resolveAndPrepend
              } = await import('../bridge/inboundAttachments.js');
              let sanitized = inboundContent;
              if (feature('KAIROS_GITHUB_WEBHOOKS')) {
                const mod = await import('../bridge/webhookSanitizer.js');
                const sanitize = (
                  mod as {
                    sanitizeInboundWebhookContent?: (
                      c: typeof inboundContent,
                    ) => typeof inboundContent
                  }
                ).sanitizeInboundWebhookContent;
                if (sanitize) {
                  sanitized = sanitize(inboundContent);
                }
              }
          ...
          if (outboundOnly) {
            ...
            if (cancelled) return;
            setMessages(prev_18 => [...prev_18, createBridgeStatusMessage(url, upgradeNudge ? 'Please upgrade to the latest version of the Deimos mobile app to see your Remote Control sessions.' : undefined)]);
