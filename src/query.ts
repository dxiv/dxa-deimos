...
const reactiveCompact = feature('REACTIVE_COMPACT')
  ? (require('./services/compact/reactiveCompact.js') as any)
  : null
...
const skillPrefetch = feature('EXPERIMENTAL_SKILL_SEARCH')
  ? (require('./services/skillSearch/prefetch.js') as any)
  : null
...
const taskSummaryModule = feature('BG_SESSIONS')
  ? (require('./utils/taskSummary.js') as any)
  : null
/* eslint-enable @typescript-eslint/no-require-imports */
...
/** String tag for where the query originated (see promptCategory). */
...
> {
  ...
  while (true) {
    ...
    for await (const update of toolUpdates) {
      if (update.message) {
        ...
        if (update.message.type === 'attachment') {
          const att = update.message.attachment as { type: string }
    ...
    const fileChangeAttachmentCount = count(
      ...
      tr =>
        tr.type === 'attachment' &&
        (tr.attachment as { type: string }).type === 'edited_text_file',
    )
