...
Press **Enter**. You should see a version like `v22.x.x` or higher.
...
## 5. Tell Deimos which AI to use (three “settings”)

Deimos needs three pieces of information for the common **OpenAI-compatible** path. You set them as **environment variables** — think of them as temporary settings for that terminal window.
...
- **Windows (PowerShell):** [Deimos quick start — Windows](quick-start-windows.md)  
- **macOS or Linux:** [Deimos quick start — macOS / Linux](quick-start-mac-linux.md)
...
When it works, Deimos opens in the terminal. Next:

1. Use the **[setup checklist](setup-checklist.md)** to confirm each prerequisite.
2. Read **[Your first minutes in Deimos](first-run.md)** — try **`/help`**, then **`/provider`**, then a short question in plain English.
...
Deimos is an independent open source project. Third-party names (OpenAI, Ollama, etc.) refer to those products so you know which service to sign up for — not an endorsement. Details: [LEGAL.md](../LEGAL.md).
