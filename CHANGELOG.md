...
- **Windows clipboard:** Native copy on Windows uses a UTF-8 temp file and PowerShell `Set-Clipboard` (same approach as [Gitlawb/openclaude](https://github.com/Gitlawb/openclaude)), replacing `clip.exe` stdin so non-ASCII text copies correctly. Added `src/ink/termio/osc.test.ts`.
