...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'deimos',
    launchCommand: 'deimos --project-aware',
    terminalName: 'deimos',
    shimEnabled: false,
    workspaceFolder: '/workspace/deimos/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/deimos/very/long/path/example-project',
    launchCwdLabel: '/workspace/deimos/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/deimos/very/long/path/example-project/.deimos-profile.json',
    workspaceProfilePath: '/workspace/deimos/very/long/path/example-project/.deimos-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the Deimos wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(html, /<span class="wordmark-accent">deimos<\/span>/);
  ...
  assert.match(
    html,
    /title="\/workspace\/deimos\/very\/long\/path\/example-project"[^>]*>\/workspace\/deimos\/very\/long\/path\/example-project<\//,
  );
});
...
test('DeimosControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { DeimosControlCenterProvider } = loadExtension();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/deimos/src/panels/control-center.js',
      workspacePath: '/workspace/deimos',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/deimos/src/panels',
      projectAwareCwdLabel: '/workspace/deimos/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/deimos',
      workspaceRootCwdLabel: '/workspace/deimos',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/deimos',
      activeFilePath: '/workspace/deimos/src/panels/control-center.js',
      workspacePath: '/workspace/deimos',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/deimos',
      projectAwareCwdLabel: '/workspace/deimos',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/deimos/src/panels',
      launchCwdLabel: '/workspace/deimos/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/deimos\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/deimos/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/deimos',
      launchCwdLabel: '/workspace/deimos',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/deimos/);
  assert.match(html, /Same workspace-root target as Launch deimos because the relative command resolves from the workspace root · \/workspace\/deimos/);
});
