...
const DEIMOS_REPO_URL = 'https://github.com/dxiv/dxa-deimos';
const DEIMOS_SETUP_URL = 'https://github.com/dxiv/dxa-deimos/blob/main/README.md#quick-start';
const PROFILE_FILE_NAME = '.deimos-profile.json';
const LEGACY_PROFILE_FILE_NAME = '.dx' + 'a-agent-profile.json';
...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('deimos');
  const launchCommand = configured.get('launchCommand', 'deimos');
  const terminalName = configured.get('terminalName', 'deimos');
...
async function launchDeimos(options = {}) {
  ...
  if (!installed) {
    ...
    if (action === 'Open Setup Guide') {
      await vscode.env.openExternal(vscode.Uri.parse(DEIMOS_SETUP_URL));
    } else if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(DEIMOS_REPO_URL));
    }
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch deimos because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="deimos wordmark"><span class="wordmark-accent">deimos</span></div>
            <div class="headline">
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the deimos repository.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and deimos commands quickly.</span>
            </button>
          </div>
        </section>
      </section>
...
class DeimosControlCenterProvider {
  constructor() {
    ...
    webviewView.webview.onDidReceiveMessage(async message => {
      switch (message?.type) {
        case 'launch':
          ...
          break;
        case 'launchRoot':
          await launchDeimos({ requireWorkspace: true });
          break;
        case 'openProfile':
...
function activate(context) {
  const provider = new DeimosControlCenterProvider();
  const refreshProvider = () => {
    void provider.refresh();
  };

  const startCommand = vscode.commands.registerCommand('deimos.start', async () => {
    await launchDeimos();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'deimos.startInWorkspaceRoot',
    async () => {
      await launchDeimos({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('deimos.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(DEIMOS_REPO_URL));
  });

  const openSetupDocsCommand = vscode.commands.registerCommand(
    'deimos.openSetupDocs',
    async () => {
      await vscode.env.openExternal(vscode.Uri.parse(DEIMOS_SETUP_URL));
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'deimos.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('deimos.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.deimos');
  });

  const providerDisposable = vscode.window.registerWebviewViewProvider(
    'deimos.controlCenter',
    provider,
  );

  const profileWatcher = vscode.workspace.createFileSystemWatcher(`**/${PROFILE_FILE_NAME}`);
  const profileWatcherLegacy = vscode.workspace.createFileSystemWatcher(`**/${LEGACY_PROFILE_FILE_NAME}`);
  ...
  context.subscriptions.push(
    ...
    providerDisposable,
    profileWatcher,
    profileWatcherLegacy,
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('deimos')) {
        refreshProvider();
      }
    }),
    ...
    profileWatcherLegacy.onDidCreate(refreshProvider),
    profileWatcherLegacy.onDidChange(refreshProvider),
    profileWatcherLegacy.onDidDelete(refreshProvider),
  );
}
